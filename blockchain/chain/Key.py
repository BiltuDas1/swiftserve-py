...
class Key:
    ...
    def load_key(self, filepath: str) -> bool:
      ...
      try:
        ...
        if not (self._key_pattern.match(lines[0]) and self._key_pattern.match(lines[-1])):
          return False
        ...
        if key_type == "PRIVATE":
          ...
          if not isinstance(loaded_key_priv, Ed25519PrivateKey):
            raise TypeError("Expected Ed25519 private key, got a different type")
          ...
          if not isinstance(loaded_key_pub, Ed25519PublicKey):
            raise TypeError("Expected Ed25519 public key, got a different type")
    ...
    def get_key(self, ip_address: str, port: int):
      ...
      key_path = f"keys/{ip_address}.pem"
