...
def tell_other_nodes(filename: str, file_details: FileInfo.FileInfo, start_range: int, end_range: int):
  ...
  if (private_key := keyring.get_private_key_raw()) is not None:
    blk = Block.Block(
      ...
      "add_file",
      File.File(
        filename, file_details.filehash, file_details.size
      ),
      machine_ip,
      private_key
    )
...
def upload(response: HttpRequest):
  ...
  if filelist.exist(uploaded_file.name) is not None:
    return JsonResponse({"status": False, "reason": "filename already exist, please choose another filename"}, status=500)
  ...
  if uploaded_file.size > (4 * 1024 * 1024):
    tell_other_nodes(uploaded_file.name, file_details, 0, (4 * 1024 * 1024))
  else:
    tell_other_nodes(uploaded_file.name, file_details, 0, uploaded_file.size)
...
def download(response: HttpRequest):
  ...
  if (filename := response.GET.get("file")) is None:
    return JsonResponse({'status': False, 'reason': 'provide a file parameter'}, status=400)
