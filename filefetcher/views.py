...
def file_response_handler(response: HttpRequest):
  ...
  if os.path.exists(os.path.join(Env.get("DOWNLOADS"), f"{work.filename}.{work.chunk}.part")):
...
def file_download_handler(response: HttpRequest):
  ...
  if (work.chunk + 1) > work.total_chunks:
    return JsonResponse({'status': False, 'reason': 'no more chunks left to download'}, status=204)
